...
int main(int argc, char *argv[])
{
	...
	nbytes = fread(&buffer[(ID_BLOCK_POSITION + 4) * BLKSIZE], 1, llen, fp);
	if(nbytes != llen)
	{
	...
	nbytes = fread(&buffer[(ID_BLOCK_POSITION + 4 + (llen + BLKSIZE) / BLKSIZE) * BLKSIZE], 1, blen, fp);
	if(nbytes != blen)
	{
	...
	idb0->flash_data_size = (llen + BLKSIZE) / BLKSIZE;
	...
	reallen = (ID_BLOCK_POSITION + 4 + (llen + BLKSIZE) / BLKSIZE) * BLKSIZE + blen;

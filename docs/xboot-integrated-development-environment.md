...
XBOOT官方提供的Eclipse IDE，其包含JRE运行环境，各种交叉工具链，各种插件，Windows版还提供了诸如`make`、`tar`、`mkdir`、`rm`等必要的命令行工具。

Eclipse拥有跨平台特性，能够在Windows、Linux、Mac等操作系统上运行。
...
> 默认Makefile为单线程编译，如感觉编译速度较慢，可在编译时传递`-j8`参数，采用多线程编译来提升编译速度。

```makefile
diff --git a/Makefile b/Makefile
index 73b2674ce..92c1d51dc 100644
--- a/Makefile
+++ b/Makefile
...
+       @TOPDIR="$(TOPDIR)" $(MAKE) -j8 -s -C src all

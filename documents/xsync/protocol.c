#include <protocol.h>

static const uint8_t crc8_table[256] = {
	,
};

static __inline uint8_t crc8_byte(uint8_t crc, const uint8_t data)
{
	return crc8_table[crc ^ data];
}

static uint8_t crc8(uint8_t crc, const uint8_t * buf, size_t len)
{
	while(len--)
		crc = crc8_byte(crc, *buf++);
	return crc;
}

static uint8_t protocal_frame_mkcrc(struct protocal_frame_t * frame)
{
	uint8_t crc = 0;

	if(frame)
	{
		uint16_t length = ((frame->length[0] << 8) | (frame->length[1] << 0));
		crc = crc8(crc, &(frame->header1), 1);
		crc = crc8(crc, &(frame->header2), 1);
		crc = crc8(crc, &(frame->length[0]), 2);
		crc = crc8(crc, &(frame->command), 1);
		crc = crc8(crc, &(frame->data[0]), length - 2);
	}
	return crc;
}

static int protocal_frame_init(struct protocal_frame_t * frame, uint8_t command, uint8_t * data, size_t size)
{
	uint16_t length;

	if(!frame)
		return -1;

	if(!data)
		size = 0;
	length = size + 2;

	frame->header1 = 0x58;
	frame->header2 = 0x78;
	frame->length[0] = (length >> 8) & 0xff;
	frame->length[1] = (length >> 0) & 0xff;
	frame->command = command;
	if(length > 2)
		memcpy(&(frame->data[0]), data, length - 2);
	frame->crc = protocal_frame_mkcrc(frame);

	return 0;
}

enum protocal_frame_state_t {
    PROTOCAL_FRAME_STATE_HEADER1,
	PROTOCAL_FRAME_STATE_HEADER2,
	PROTOCAL_FRAME_STATE_LENGTH1,
	PROTOCAL_FRAME_STATE_LENGTH2,
	PROTOCAL_FRAME_STATE_COMMAND,
	PROTOCAL_FRAME_STATE_DATA,
	PROTOCAL_FRAME_STATE_CRC,
};

static int protocal_frame_push_byte(struct protocal_frame_t * frame, uint8_t c)
{
	static enum protocal_frame_state_t state = PROTOCAL_FRAME_STATE_HEADER1;
	static uint8_t buffer[2 + 2 + 1 + 4096 + 1];
	static size_t index = 0;
	uint16_t length;

	buffer[index++] = c;

	switch(state)
	{
	case PROTOCAL_FRAME_STATE_HEADER1:
		if(c == 0x58)
		{
			state = PROTOCAL_FRAME_STATE_HEADER2;
		}
		else
		{
			index = 0;
			state = PROTOCAL_FRAME_STATE_HEADER1;
		}
		break;

	case PROTOCAL_FRAME_STATE_HEADER2:
		if(c == 0x78)
		{
			state = PROTOCAL_FRAME_STATE_LENGTH1;
		}
		else
		{
			index = 0;
			state = PROTOCAL_FRAME_STATE_HEADER1;
		}
		break;

	case PROTOCAL_FRAME_STATE_LENGTH1:
		state = PROTOCAL_FRAME_STATE_LENGTH2;
		break;

	case PROTOCAL_FRAME_STATE_LENGTH2:
		state = PROTOCAL_FRAME_STATE_COMMAND;
		break;

	case PROTOCAL_FRAME_STATE_COMMAND:
		state = PROTOCAL_FRAME_STATE_DATA;
		length = (buffer[2] << 8) | (buffer[3] << 0);
		if(index >= length + 3)
			state = PROTOCAL_FRAME_STATE_CRC;
		break;

	case PROTOCAL_FRAME_STATE_DATA:
		length = (buffer[2] << 8) | (buffer[3] << 0);
		if(index >= length + 3)
			state = PROTOCAL_FRAME_STATE_CRC;
		break;

	case PROTOCAL_FRAME_STATE_CRC:
		length = (buffer[2] << 8) | (buffer[3] << 0);
		index = 0;
		state = PROTOCAL_FRAME_STATE_HEADER1;
		if(protocal_frame_init(frame, buffer[4], &buffer[5], (size_t)(length - 2)) == 0)
		{
			if(frame->crc == c)
			{
				return 0;
			}
		}
		break;

	default:
		index = 0;
		state = PROTOCAL_FRAME_STATE_HEADER1;
		break;
	}

	return -1;
}

static int protocal_frame_get(struct interface_t * iface, struct protocal_frame_t * frame, int timeout)
{
	uint64_t time, end;
	uint8_t c;
	int ret = -1;

	end = interface_time(iface) + timeout;
	do {
		if(interface_read(iface, &c, 1) == 1)
			ret = protocal_frame_push_byte(frame, c);
		time = interface_time(iface);
	} while((ret < 0) && (time <= end));

	return ret;
}

static int protocal_frame_put(struct interface_t * iface, struct protocal_frame_t * frame)
{
	uint16_t length;

	if(!frame)
		return -1;

	length = ((frame->length[0] << 8) | (frame->length[1] << 0));
	interface_write(iface, &(frame->header1), 1);
	interface_write(iface, &(frame->header2), 1);
	interface_write(iface, &(frame->length), 2);
	interface_write(iface, &(frame->command), 1);
	if(length > 2)
		interface_write(iface, &(frame->data[0]), length - 2);
	interface_write(iface, &(frame->crc), 1);

	return 0;
}

int protocal_frame_transform(struct interface_t * iface, struct protocal_frame_t * request, struct protocal_frame_t * response, int timeout)
{
	if(protocal_frame_put(iface, request) < 0)
		return -1;
	return protocal_frame_get(iface, response, timeout);
}

...
static __init void realview_serial_dev_init(void)
{
	...
	{
		DEBUG_E("can't get the clock of \'uclk\'");
		return;
	}
	...
	for(i = 0; i < ARRAY_SIZE(realview_serial_driver); i++)
	{
		...
		if(param)
			...
			DEBUG_W("can't get the platform device data of \'%s\', use default parameter", realview_serial_driver[i].info->name);

		if(!register_serial(&realview_serial_driver[i]))
			DEBUG_E("failed to register serial driver '%s'", realview_serial_driver[i].info->name);
	}
}
...
static __exit void realview_serial_dev_exit(void)
{
	...
	for(i = 0; i < ARRAY_SIZE(realview_serial_driver); i++)
	{
		if(!unregister_serial(&realview_serial_driver[i]))
			DEBUG_E("failed to unregister serial driver '%s'", realview_serial_driver[i].info->name);
	}
}

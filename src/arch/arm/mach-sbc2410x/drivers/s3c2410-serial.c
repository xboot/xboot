...
static x_s32 s3c2410_uart0_read(x_u8 * buf, x_s32 count)
{
	x_s32 i;
...
static x_s32 s3c2410_uart0_write(const x_u8 * buf, x_s32 count)
{
	x_s32 i;
...
static x_s32 s3c2410_uart1_read(x_u8 * buf, x_s32 count)
{
	x_s32 i;
...
static x_s32 s3c2410_uart1_write(const x_u8 * buf, x_s32 count)
{
	x_s32 i;
...
static x_s32 s3c2410_uart2_read(x_u8 * buf, x_s32 count)
{
	x_s32 i;
...
static x_s32 s3c2410_uart2_write(const x_u8 * buf, x_s32 count)
{
	x_s32 i;
...
static __init void s3c2410_serial_dev_init(void)
{
	...
	for(i = 0; i < ARRAY_SIZE(s3c2410_uart_driver); i++)
	{
		...
		if(param)
			...
			DEBUG_W("can't get the platform device data of \'%s\', use default parameter", s3c2410_uart_driver[i].info->name);

		if(!register_serial(&s3c2410_uart_driver[i]))
			DEBUG_E("failed to register serial driver '%s'", s3c2410_uart_driver[i].info->name);
	}
}
...
static __exit void s3c2410_serial_dev_exit(void)
{
	...
	for(i = 0; i < ARRAY_SIZE(s3c2410_uart_driver); i++)
	{
		if(!unregister_serial(&s3c2410_uart_driver[i]))
			DEBUG_E("failed to unregister serial driver '%s'", s3c2410_uart_driver[i].info->name);
	}
}

module_init(s3c2410_serial_dev_init, LEVEL_DRIVER);
module_exit(s3c2410_serial_dev_exit, LEVEL_DRIVER);

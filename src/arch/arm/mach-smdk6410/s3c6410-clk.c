/*
 * arch/arm/mach-smdk6410/s3c6410-clk.c
 *
...
static __init void s3c6410_clk_init(void)
{
	...
	for(i=0; i< ARRAY_SIZE(s3c6410_clocks); i++)
	{
		...
		{
			DEBUG_E("failed to register clock '%s'", s3c6410_clocks[i].name);
		}
	}
}
...
static __exit void s3c6410_clk_exit(void)
{
	...
	for(i=0; i< ARRAY_SIZE(s3c6410_clocks); i++)
	{
		...
		{
			DEBUG_E("failed to unregister clock '%s'", s3c6410_clocks[i].name);
		}
	}
}

module_init(s3c6410_clk_init, LEVEL_MACH_RES);
module_exit(s3c6410_clk_exit, LEVEL_MACH_RES);

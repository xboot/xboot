...
/*	if(!clk_get_rate("psys-pclk", 0))
	...
	{
		param = (struct serial_parameter *)resource_get_data(s5pv310_uart_driver[i].info->name);
		if(param)
...
module_init(s5pv310_serial_dev_init, LEVEL_DRIVER);
module_exit(s5pv310_serial_dev_exit, LEVEL_DRIVER);

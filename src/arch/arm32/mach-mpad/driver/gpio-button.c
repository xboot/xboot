...
static void button_timer_function(u32_t data)
{
	...
	if(readl(S5PV210_GPH3DAT) & (0x1<<5))
	{
		key &= ~(0x1 << 2);
	}
	else
...
static bool_t button_probe(struct input * input)
{
	...
	/* set GPH3_5 intput and pull up */
	writel(S5PV210_GPH3CON, (readl(S5PV210_GPH3CON) & ~(0xf<<20)) | (0x0<<20));
	writel(S5PV210_GPH3PUD, (readl(S5PV210_GPH3PUD) & ~(0x3<<10)) | (0x2<<10));

/*
 * driver/s5pv210-serial.c
 *
 * s5pv210 on chip serial drivers.
 *
 ...
 */
static struct serial_parameter_t uart_param[4] = {
	[0] = {
		.baud_rate		= B115200,
 ...
 * serial information.
 */
static struct serial_info_t uart_info[4] = {
	[0] = {
		.name			= "uart0",
		.desc			= "s5pv210 onchip serial 0",
		.parameter		= &uart_param[0],
	},
	[1] = {
		.name			= "uart1",
		.desc			= "s5pv210 onchip serial 1",
		.parameter		= &uart_param[1],
	},
	[2] = {
		.name			= "uart2",
		.desc			= "s5pv210 onchip serial 2",
		.parameter		= &uart_param[2],
	},
	[3] = {
		.name			= "uart3",
		.desc			= "s5pv210 onchip serial 3",
...
static void s5pv210_uart0_init(void)
{
	...
	s5pv210_ioctl( 0, IOCTL_WR_SERIAL_BAUD_RATE, (void *)(&(uart_param[0].baud_rate)) );
...
static void s5pv210_uart1_init(void)
{
	...
	/* configure uart parameter */
	s5pv210_ioctl( 1, IOCTL_WR_SERIAL_BAUD_RATE, (void *)(&(uart_param[1].baud_rate)) );
...
static void s5pv210_uart2_init(void)
{
	...
	/* configure uart parameter */
	s5pv210_ioctl( 2, IOCTL_WR_SERIAL_BAUD_RATE, (void *)(&(uart_param[2].baud_rate)) );
...
static void s5pv210_uart3_init(void)
{
	...
	/* configure uart parameter */
	s5pv210_ioctl( 3, IOCTL_WR_SERIAL_BAUD_RATE, (void *)(&(uart_param[3].baud_rate)) );
...
static struct serial_driver_t s5pv210_uart_driver[4] = {
	[0] = {
		.info	= &uart_info[0],
		.init	= s5pv210_uart0_init,
		.exit	= s5pv210_uart0_exit,
		.read	= s5pv210_uart0_read,
		.write	= s5pv210_uart0_write,
		.ioctl	= s5pv210_uart0_ioctl,
	},
	[1] = {
		.info	= &uart_info[1],
		.init	= s5pv210_uart1_init,
		.exit	= s5pv210_uart1_exit,
		.read	= s5pv210_uart1_read,
		.write	= s5pv210_uart1_write,
		.ioctl	= s5pv210_uart1_ioctl,
	},
	[2] = {
		.info	= &uart_info[2],
		.init	= s5pv210_uart2_init,
		.exit	= s5pv210_uart2_exit,
		.read	= s5pv210_uart2_read,
		.write	= s5pv210_uart2_write,
		.ioctl	= s5pv210_uart2_ioctl,
	},
	[3] = {
		.info	= &uart_info[3],
		.init	= s5pv210_uart3_init,
		.exit	= s5pv210_uart3_exit,
		.read	= s5pv210_uart3_read,
		.write	= s5pv210_uart3_write,
		.ioctl	= s5pv210_uart3_ioctl,
	}
};
...
static __init void s5pv210_serial_dev_init(void)
{
	...
	{
		LOG("can't get the clock of \'psys-pclk\'");
		return;
	}
	...
	{
		param = (struct serial_parameter_t *)resource_get_data(s5pv210_uart_driver[i].info->name);
		if(param)
			memcpy(s5pv210_uart_driver[i].info->parameter, param, sizeof(struct serial_parameter_t));
		else
			LOG("can't get the resource of \'%s\', use default parameter", s5pv210_uart_driver[i].info->name);

		if(!register_serial(&s5pv210_uart_driver[i]))
			LOG("failed to register serial driver '%s'", s5pv210_uart_driver[i].info->name);
	}
}
...
static __exit void s5pv210_serial_dev_exit(void)
{
	...
	for(i = 0; i < ARRAY_SIZE(s5pv210_uart_driver); i++)
	{
		if(!unregister_serial(&s5pv210_uart_driver[i]))
			LOG("failed to unregister serial driver '%s'", s5pv210_uart_driver[i].info->name);
	}
}

device_initcall(s5pv210_serial_dev_init);
device_exitcall(s5pv210_serial_dev_exit);

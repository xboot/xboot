...
static int ccu_set_pll_ddr_clk(int index, struct dram_param_t * para)
{
	...
	val |= 0xc0000000;
	val &= 0xdfffffff;
	write32((R528_CCU_BASE + CCU_PLL_DDR_CTRL_REG), val | 0x20000000);
	while((read32((R528_CCU_BASE + CCU_PLL_DDR_CTRL_REG)) & 0x10000000) == 0);
	sdelay(20);
...
{
	clrbits_le32((R528_CCU_BASE + CCU_MBUS_CLK_REG), (1 << 30));
	clrbits_le32((R528_CCU_BASE + CCU_DRAM_BGR_REG), 0x10001);
	clrsetbits_le32((R528_CCU_BASE + CCU_DRAM_CLK_REG), (1 << 31) | (1 << 30), (1 << 27));
	sdelay(10);
	para->dram_clk = ccu_set_pll_ddr_clk(0, para) / 2;
	sdelay(100);
	dram_disable_all_master();
	setbits_le32((R528_CCU_BASE + CCU_DRAM_BGR_REG), (1 << 16));
	setbits_le32((R528_CCU_BASE + CCU_MBUS_CLK_REG), (1 << 30));
	setbits_le32((R528_CCU_BASE + CCU_DRAM_CLK_REG), (1 << 30));
	sdelay(5);
	setbits_le32((R528_CCU_BASE + CCU_DRAM_BGR_REG), (1 << 0));
	setbits_le32((R528_CCU_BASE + CCU_DRAM_CLK_REG), (1 << 31) | (1 << 27));
	sdelay(5);

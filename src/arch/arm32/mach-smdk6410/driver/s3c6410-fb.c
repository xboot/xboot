...
static void fb_init(struct fb * fb)
{
	...
	/* gpn5 high level for enable lcd power */
	writel(S3C6410_GPNCON, (readl(S3C6410_GPNCON) & ~(0x3<<10)) | (0x1<<10));
	writel(S3C6410_GPNPUD, (readl(S3C6410_GPNPUD) & ~(0x3<<10)) | (0x2<<10));
	writel(S3C6410_GPNDAT, (readl(S3C6410_GPNDAT) & ~(0x1<<5)) | (0x1<<5));

	/* set gpf13 (lcd pannel on) output and pull up and high level */
	writel(S3C6410_GPFCON, (readl(S3C6410_GPFCON) & ~(0x3<<26)) | (0x1<<26));
	writel(S3C6410_GPFPUD, (readl(S3C6410_GPFPUD) & ~(0x3<<26)) | (0x2<<26));
	writel(S3C6410_GPFDAT, (readl(S3C6410_GPFDAT) & ~(0x1<<13)) | (0x1<<13));
...
static void fb_present(struct fb * fb, int index)
{
	...
	writel(S3C6410_VIDW00ADD0B0, ((u32_t)fb->info->surface[index]->pixels));
	writel(S3C6410_VIDW00ADD0B1, ((u32_t)fb->info->surface[index]->pixels));
	writel(S3C6410_VIDW00ADD1B0, (((u32_t)fb->info->surface[index]->pixels) + LCD_WIDTH*LCD_HEIGHT*LCD_BPP/8)& 0x00ffffff);
	writel(S3C6410_VIDW00ADD1B1, (((u32_t)fb->info->surface[index]->pixels) + LCD_WIDTH*LCD_HEIGHT*LCD_BPP/8)& 0x00ffffff);
}

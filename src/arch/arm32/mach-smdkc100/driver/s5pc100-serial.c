...
static struct serial_driver_t s5pc100_uart_driver[4] = {
	[0] = {
		.info	= &uart_info[0],
...
static __init void s5pc100_serial_dev_init(void)
{
	...
	{
		param = (struct serial_parameter_t *)resource_get_data(s5pc100_uart_driver[i].info->name);
		if(param)
			memcpy(s5pc100_uart_driver[i].info->parameter, param, sizeof(struct serial_parameter_t));
		else
			LOG("can't get the resource of \'%s\', use default parameter", s5pc100_uart_driver[i].info->name);

		if(!register_serial(&s5pc100_uart_driver[i]))
			LOG("failed to register serial driver '%s'", s5pc100_uart_driver[i].info->name);
	}
}
...
static __exit void s5pc100_serial_dev_exit(void)
{
	...
	for(i = 0; i < ARRAY_SIZE(s5pc100_uart_driver); i++)
	{
		if(!unregister_serial(&s5pc100_uart_driver[i]))
			LOG("failed to unregister serial driver '%s'", s5pc100_uart_driver[i].info->name);
	}
}

/*
 * driver/s5pv310-serial.c
 *
 * s5pv310 on chip serial drivers.
 *
 ...
 */
static struct serial_parameter_t uart_param[5] = {
	[0] = {
		.baud_rate		= B115200,
 ...
 * serial information.
 */
static struct serial_info_t uart_info[5] = {
	[0] = {
		.name			= "uart0",
	...
	[1] = {
		.name			= "uart1",
		.desc			= "s5pv310 onchip serial 1",
		.parameter		= &uart_param[1],
	},
	[2] = {
		.name			= "uart2",
		.desc			= "s5pv310 onchip serial 2",
 ...
 */
static int s5pv310_ioctl(u32_t ch, int cmd, void * arg)
{
...
static void s5pv310_uart0_init(void)
{
	...
	writel(S5PV310_UFCON0, 0x00000000);
	writel(S5PV310_UMON0, 0x00000000);

	/* configure uart parameter */
	s5pv310_ioctl( 0, IOCTL_WR_SERIAL_BAUD_RATE, (void *)(&(uart_param[0].baud_rate)) );
	s5pv310_ioctl( 0, IOCTL_WR_SERIAL_DATA_BITS, (void *)(&(uart_param[0].data_bit)) );
	s5pv310_ioctl( 0, IOCTL_WR_SERIAL_PARITY_BIT, (void *)(&(uart_param[0].parity)) );
	s5pv310_ioctl( 0, IOCTL_WR_SERIAL_STOP_BITS, (void *)(&(uart_param[0].stop_bit)) );
}

static void s5pv310_uart0_exit(void)
{
	return;
}

static ssize_t s5pv310_uart0_read(u8_t * buf, size_t count)
{
	ssize_t i;

	for(i = 0; i < count; i++)
	{
		if( (readl(S5PV310_UTRSTAT0) & S5PV310_UTRSTAT_RXDR) )
			buf[i] = readb(S5PV310_URXH0);
		else
			break;
	}
	return i;
...
static ssize_t s5pv310_uart0_write(const u8_t * buf, size_t count)
{
	ssize_t i;
	...
	for(i = 0; i < count; i++)
	{
		...
		while( !(readl(S5PV310_UTRSTAT0) & S5PV310_UTRSTAT_TXE) );

		/* transmit a character */
		writeb(S5PV310_UTXH0, buf[i]);
	}
	return i;
}

static int s5pv310_uart0_ioctl(int cmd, void * arg)
{
	return (s5pv310_ioctl(0, cmd, arg));
}
...
static void s5pv310_uart1_init(void)
{
	...
	writel(S5PV310_UFCON1, 0x00000000);
	writel(S5PV310_UMON1, 0x00000000);

	/* configure uart parameter */
	s5pv310_ioctl( 1, IOCTL_WR_SERIAL_BAUD_RATE, (void *)(&(uart_param[1].baud_rate)) );
	s5pv310_ioctl( 1, IOCTL_WR_SERIAL_DATA_BITS, (void *)(&(uart_param[1].data_bit)) );
	s5pv310_ioctl( 1, IOCTL_WR_SERIAL_PARITY_BIT, (void *)(&(uart_param[1].parity)) );
	s5pv310_ioctl( 1, IOCTL_WR_SERIAL_STOP_BITS, (void *)(&(uart_param[1].stop_bit)) );
}

static void s5pv310_uart1_exit(void)
{
	return;
}

static ssize_t s5pv310_uart1_read(u8_t * buf, size_t count)
{
	ssize_t i;

	for(i = 0; i < count; i++)
	{
		if( (readl(S5PV310_UTRSTAT1) & S5PV310_UTRSTAT_RXDR) )
			buf[i] = readb(S5PV310_URXH1);
		else
			break;
	}
	return i;
...
static ssize_t s5pv310_uart1_write(const u8_t * buf, size_t count)
{
	ssize_t i;
	...
	{
		/* wait for transmit buffer & shifter register empty */
		while( !(readl(S5PV310_UTRSTAT1) & S5PV310_UTRSTAT_TXE) );

		/* transmit a character */
		writeb(S5PV310_UTXH1, buf[i]);
	}
	return i;
}

static int s5pv310_uart1_ioctl(int cmd, void * arg)
{
	return (s5pv310_ioctl(1, cmd, arg));
}
...
static void s5pv310_uart2_init(void)
{
	...
	s5pv310_ioctl( 2, IOCTL_WR_SERIAL_DATA_BITS, (void *)(&(uart_param[2].data_bit)) );
	s5pv310_ioctl( 2, IOCTL_WR_SERIAL_PARITY_BIT, (void *)(&(uart_param[2].parity)) );
	s5pv310_ioctl( 2, IOCTL_WR_SERIAL_STOP_BITS, (void *)(&(uart_param[2].stop_bit)) );
}

static void s5pv310_uart2_exit(void)
{
	return;
}

static ssize_t s5pv310_uart2_read(u8_t * buf, size_t count)
{
	ssize_t i;

	for(i = 0; i < count; i++)
	{
		if( (readl(S5PV310_UTRSTAT2) & S5PV310_UTRSTAT_RXDR) )
			buf[i] = readb(S5PV310_URXH2);
		else
			break;
	}
	return i;
...
static ssize_t s5pv310_uart2_write(const u8_t * buf, size_t count)
{
	ssize_t i;
	...
	{
		/* wait for transmit buffer & shifter register empty */
		while( !(readl(S5PV310_UTRSTAT2) & S5PV310_UTRSTAT_TXE) );

		/* transmit a character */
		writeb(S5PV310_UTXH2, buf[i]);
	}
	return i;
}

static int s5pv310_uart2_ioctl(int cmd, void * arg)
{
	return (s5pv310_ioctl(2, cmd, arg));
}
...
static void s5pv310_uart3_init(void)
{
	...
	s5pv310_ioctl( 3, IOCTL_WR_SERIAL_DATA_BITS, (void *)(&(uart_param[3].data_bit)) );
	s5pv310_ioctl( 3, IOCTL_WR_SERIAL_PARITY_BIT, (void *)(&(uart_param[3].parity)) );
	s5pv310_ioctl( 3, IOCTL_WR_SERIAL_STOP_BITS, (void *)(&(uart_param[3].stop_bit)) );
}

static void s5pv310_uart3_exit(void)
{
	return;
}

static ssize_t s5pv310_uart3_read(u8_t * buf, size_t count)
{
	ssize_t i;

	for(i = 0; i < count; i++)
	{
		if( (readl(S5PV310_UTRSTAT3) & S5PV310_UTRSTAT_RXDR) )
			buf[i] = readb(S5PV310_URXH3);
		else
			break;
	}
	return i;
...
static ssize_t s5pv310_uart3_write(const u8_t * buf, size_t count)
{
	ssize_t i;
	...
	{
		/* wait for transmit buffer & shifter register empty */
		while( !(readl(S5PV310_UTRSTAT3) & S5PV310_UTRSTAT_TXE) );

		/* transmit a character */
		writeb(S5PV310_UTXH3, buf[i]);
	}
	return i;
}

static int s5pv310_uart3_ioctl(int cmd, void * arg)
{
...
static struct serial_driver_t s5pv310_uart_driver[5] = {
	[0] = {
		.info	= &uart_info[0],
		.init	= s5pv310_uart0_init,
		.exit	= s5pv310_uart0_exit,
		.read	= s5pv310_uart0_read,
		.write	= s5pv310_uart0_write,
		.ioctl	= s5pv310_uart0_ioctl,
	},
	[1] = {
		.info	= &uart_info[1],
		.init	= s5pv310_uart1_init,
		.exit	= s5pv310_uart1_exit,
		.read	= s5pv310_uart1_read,
		.write	= s5pv310_uart1_write,
		.ioctl	= s5pv310_uart1_ioctl,
	},
	[2] = {
		.info	= &uart_info[2],
		.init	= s5pv310_uart2_init,
		.exit	= s5pv310_uart2_exit,
		.read	= s5pv310_uart2_read,
		.write	= s5pv310_uart2_write,
		.ioctl	= s5pv310_uart2_ioctl,
	},
	[3] = {
		.info	= &uart_info[3],
		.init	= s5pv310_uart3_init,
		.exit	= s5pv310_uart3_exit,
...
static __init void s5pv310_serial_dev_init(void)
{
	struct serial_parameter_t * param;
	u32_t i;
	...
	{
		param = (struct serial_parameter_t *)resource_get_data(s5pv310_uart_driver[i].info->name);
		if(param)
			memcpy(s5pv310_uart_driver[i].info->parameter, param, sizeof(struct serial_parameter_t));
		else
...
static __exit void s5pv310_serial_dev_exit(void)
{
	u32_t i;
...
device_initcall(s5pv310_serial_dev_init);
device_exitcall(s5pv310_serial_dev_exit);

...
static void exynos4412_gpiochip_set_cfg(struct gpiochip_t * chip, int offset, int cfg)
{
	...
	offset <<= 0x2;
	val = read32(dat->regbase + EXYNOS4412_GPIO_CON);
	val &= ~(0xf << offset);
	val |= cfg << offset;
	write32(dat->regbase + EXYNOS4412_GPIO_CON, val);
}
...
static void exynos4412_gpiochip_set_pull(struct gpiochip_t * chip, int offset, enum gpio_pull_t pull)
{
	...
	offset <<= 0x1;
	val = read32(dat->regbase + EXYNOS4412_GPIO_PUD);
	val &= ~(0x3 << offset);
	val |= p << offset;
	write32(dat->regbase + EXYNOS4412_GPIO_PUD, val);
}
...
static void exynos4412_gpiochip_set_drv(struct gpiochip_t * chip, int offset, enum gpio_drv_t drv)
{
	...
	offset <<= 0x1;
	val = read32(dat->regbase + EXYNOS4412_GPIO_DRV);
	val &= ~(0x3 << offset);
	val |= d << offset;
	write32(dat->regbase + EXYNOS4412_GPIO_DRV, val);
}
...
static void exynos4412_gpiochip_set_value(struct gpiochip_t * chip, int offset, int value)
{
	...
	val = read32(dat->regbase + EXYNOS4412_GPIO_DAT);
	val &= ~(1 << offset);
	val |= (!!value) << offset;
	write32(dat->regbase + EXYNOS4412_GPIO_DAT, val);
}
...
static const char * exynos4412_gpiochip_to_irq(struct gpiochip_t * chip, int offset)
{
	return 0;
}
...
static __init void exynos4412_gpiochip_init(void)
{
	...
	for(i = 0; i < ARRAY_SIZE(gpiochip_datas); i++)
	{
		...
		chip->to_irq = exynos4412_gpiochip_to_irq;

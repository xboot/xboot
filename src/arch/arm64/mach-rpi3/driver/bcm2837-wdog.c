...
#include <bcm2837-wdog.h>

struct bcm2837_wdog_pdata_t {
	virtual_addr_t virt;
	int start;
};
...
{
	struct bcm2837_wdog_pdata_t * pdat = (struct bcm2837_wdog_pdata_t *)watchdog->priv;
	u32_t val;
...
static bool_t bcm2837_register_wdog(struct resource_t * res)
{
	struct bcm2837_wdog_data_t * rdat = (struct bcm2837_wdog_data_t *)res->data;
	struct bcm2837_wdog_pdata_t * pdat;
	struct watchdog_t * wdog;
	char name[64];

	pdat = malloc(sizeof(struct bcm2837_wdog_pdata_t));
	if(!pdat)
		return FALSE;
...
static bool_t bcm2837_unregister_wdog(struct resource_t * res)
{
	struct watchdog_t * wdog;
	char name[64];
...
static __init void bcm2837_watchdog_init(void)
{
	resource_for_each("bcm2837-wdog", bcm2837_register_wdog);
}

static __exit void bcm2837_watchdog_exit(void)
{
	resource_for_each("bcm2837-wdog", bcm2837_unregister_wdog);
}

device_initcall(bcm2837_watchdog_init);
device_exitcall(bcm2837_watchdog_exit);

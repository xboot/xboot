...
static bool_t virt_uart_setup(struct uart_t * uart, enum baud_rate_t baud, enum data_bits_t data, enum parity_bits_t parity, enum stop_bits_t stop)
{
	struct resource_t * res = (struct resource_t *)uart->priv;
	struct virt_uart_data_t * dat = (struct virt_uart_data_t *)res->data;
...
static void virt_uart_init(struct uart_t * uart)
{
	struct resource_t * res = (struct resource_t *)uart->priv;
	struct virt_uart_data_t * dat = (struct virt_uart_data_t *)res->data;
	...
	/* Configure uart */
	virt_uart_setup(uart, dat->baud, dat->data, dat->parity, dat->stop);

	/* Enable uart */
	write32(phys_to_virt(dat->regbase + UART_CR), (1 << 0) | (1 << 8) | (1 << 9));
}

static void virt_uart_exit(struct uart_t * uart)
{
	clk_disable("uclk");
}

static ssize_t virt_uart_read(struct uart_t * uart, u8_t * buf, size_t count)
{
	struct resource_t * res = (struct resource_t *)uart->priv;
	struct virt_uart_data_t * dat = (struct virt_uart_data_t *)res->data;
	ssize_t i;
...
static ssize_t virt_uart_write(struct uart_t * uart, const u8_t * buf, size_t count)
{
	struct resource_t * res = (struct resource_t *)uart->priv;
	struct virt_uart_data_t * dat = (struct virt_uart_data_t *)res->data;
	ssize_t i;
...
static bool_t virt_register_bus_uart(struct resource_t * res)
{
	struct uart_t * uart;
	char name[64];
	...
	uart->init = virt_uart_init;
	uart->exit = virt_uart_exit;
	uart->read = virt_uart_read;
	uart->write = virt_uart_write;
	uart->setup = virt_uart_setup;
	uart->priv = res;
...
static bool_t virt_unregister_bus_uart(struct resource_t * res)
{
	struct uart_t * uart;
	char name[64];
...
static __init void virt_bus_uart_init(void)
{
	resource_for_each_with_name("virt-uart", virt_register_bus_uart);
}

static __exit void virt_bus_uart_exit(void)
{
	resource_for_each_with_name("virt-uart", virt_unregister_bus_uart);
}

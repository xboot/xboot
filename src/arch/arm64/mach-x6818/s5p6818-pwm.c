...
static struct s5p6818_pwm_data_t datas[] = {
	{
		.name	= "pwm0",
		.ch		= 0,
		.pin	= S5P6818_GPIOD(1),
		.pincfg	= 0x1,
		.phys	= S5P6818_PWM_BASE,
	}, {
		.name	= "pwm1",
		.ch		= 1,
		.pin	= S5P6818_GPIOC(13),
		.pincfg	= 0x2,
		.phys	= S5P6818_PWM_BASE,
	},
	{
...
{
	struct s5p6818_pwm_pdata_t * pdat = (struct s5p6818_pwm_pdata_t *)pwm->priv;
...
{
	struct s5p6818_pwm_pdata_t * pdat;
	struct pwm_t * pwm;
	int i;
	...
	{
		pdat = malloc(sizeof(struct s5p6818_pwm_pdata_t));
		if(!pdat)
			continue;

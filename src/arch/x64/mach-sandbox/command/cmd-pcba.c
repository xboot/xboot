...
static void xui_badge_result(struct xui_context_t * ctx, int result)
{
	...
	{
	case 0:
		xui_icon(ctx, 0xe5c9, &(struct color_t){0xff, 0x5b, 0x5b, 0xff});
		break;
	case 1:
		xui_icon(ctx, 0xe86c, &(struct color_t){0x10, 0xc4, 0x69, 0xff});
		break;
	default:
		xui_icon(ctx, 0xe15c, &(struct color_t){0xcf, 0xcf, 0xcf, 0xff});
		break;
	}
}

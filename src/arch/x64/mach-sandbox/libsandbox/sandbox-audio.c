...
void * sandbox_audio_playback_start(int rate, int fmt, int ch)
{
	...
	if(snd_pcm_open(&ctx->handle, "default", SND_PCM_STREAM_PLAYBACK, SND_PCM_NONBLOCK) < 0)
	{
		free(ctx);
		return NULL;
	}
...
int sandbox_audio_playback_write(void * context, void * buf, int len)
{
	...
	if(ctx && ctx->handle)
	{
		int frames = sandbox_min((int)(len / ctx->bytes_per_frame), (int)ctx->frames);
		int ret = snd_pcm_writei(ctx->handle, buf, frames);
		if(ret > 0)
			return ret * ctx->bytes_per_frame;
		else if(ret == -EPIPE)
		{
...
void * sandbox_audio_capture_start(int rate, int fmt, int ch)
{
	...
	if(snd_pcm_open(&ctx->handle, "default", SND_PCM_STREAM_CAPTURE, SND_PCM_NONBLOCK) < 0)
	{
		free(ctx);
		return NULL;
	}
...
int sandbox_audio_capture_read(void * context, void * buf, int len)
{
	...
	{
		int frames = sandbox_min((int)(len / ctx->bytes_per_frame), (int)ctx->frames);
		int ret = snd_pcm_readi(ctx->handle, buf, frames);
		if(ret > 0)
			return ret * ctx->bytes_per_frame;
		else if(ret == -EPIPE)
		{

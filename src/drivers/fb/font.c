...
static const x_u8 unkown_font_data_8x8[16][8] = {
	[0x0] = {
		0x38,	/* 00111000 */
		0x6c,	/* 01101100 */
		0xc6,	/* 11000110 */
...
static struct font_glyph * unkown_font_glyph(x_u32 code)
{
	...
	idx = (code >> 12) & 0xf;
	for(i = 0; i < 8; i++)
		data[0 + i*2] = ~unkown_font_data_8x8[idx][i];

	idx = (code >> 8) & 0xf;
	for(i = 0; i < 8; i++)
		data[1 + i*2] = ~unkown_font_data_8x8[idx][i];

	idx = (code >> 4) & 0xf;
	for(i = 0; i < 8; i++)
		data[16 + i*2] = ~unkown_font_data_8x8[idx][i];

	idx = (code >> 0) & 0xf;
	for(i = 0; i < 8; i++)
		data[17 + i*2] = ~unkown_font_data_8x8[idx][i];
...
static x_bool register_font(struct font * font)
{
	...
	if(!font->name || font_get(font->name))
	{
		free(list);
		return FALSE;
	}

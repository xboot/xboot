...
static void __common_event_listener_callback(struct event_t * event, void * data)
{
	struct event_callback_data_t * d = (struct event_callback_data_t *)data;
	struct event_t * e;
	...
	luaL_setmetatable(d->l, TYPE_EVENT);
...
static int l_event_listener_new(lua_State * L)
{
	...
	struct event_callback_data_t * d;
	...
	d = malloc(sizeof(struct event_callback_data_t));
	if(!d)
		return 0;
	d->l = L;
	d->r = r;
	...
	el->type = type;
	el->callback = __common_event_listener_callback;
	el->data = d;
	luaL_setmetatable(L, TYPE_EVENT_LISTENER);
...
{
	struct event_listener_t * el = lua_touserdata(L, 1);
	struct event_callback_data_t * d;
...
int luaopen_event_listener(lua_State * L)
{
	...
	luaL_newmetatable(L, TYPE_EVENT_LISTENER);
	lua_pushvalue(L, -1);

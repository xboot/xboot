...
struct render_t
{
	...
	void (*shape_square)(struct surface_t * s, struct region_t * clip, int x, int y, int w, int h);
...
static inline void surface_shape_square(struct surface_t * s, struct region_t * clip, int x, int y, int w, int h)
{
	s->r->shape_square(s, clip, x, y, w, h);
}

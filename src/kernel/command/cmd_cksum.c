/*
 * kernel/command/cmd_cksum.c
 *
 * Copyright (c) 2007-2012  jianjun jiang <jjjstudio@gmail.com>
...
static u32_t crc32(u8_t *p, s32_t len, u32_t *crc)
{
    ...
    *crc = 0;
    ...
    while (len > 0)
    {
    	*crc = (*crc << 8) ^ crc32_table[((*crc >> 24) ^ len) & 0xff];
    	len >>= 8;
    }
...
static struct command cksum_cmd = {
	...
	.usage		= "cksum <address> <size>\r\n",
	.help		= "    computes 32-bit CRC for the specified memory area.\r\n"
};

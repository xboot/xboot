...
void queue_free(struct queue_t * q, void (*iteration)(struct queue_node_t *))
{
	if(q)
	{
		queue_clear(q, iteration);
		free(q);
	}
}
EXPORT_SYMBOL(queue_free);

void queue_clear(struct queue_t * q, void (*iteration)(struct queue_node_t *))
{

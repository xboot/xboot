...
static int currentline(lua_State * L, CallInfo * ci)
{
	...
	if(pc < 0)			/* only active lua functions have current-line information */
		return -1;
	else
...
int luaG_checkopenop(Instruction i)
{
	...
	default:			/* invalid instruction after an open call */
		return 0;
	}
}
...
void luaG_typeerror(lua_State * L, const TValue * o, const char * op)
{
	...
	const char *kind = (isinstack(L->ci, o)) ? getobjname(L, L->ci,
			cast_int(o - L->base), &name) : NULL;
	if (kind)

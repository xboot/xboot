...
static void errorlimit(FuncState *fs, int limit, const char *what) {
	const char *msg = (fs->f->linedefined == 0) ? luaO_pushfstring(fs->L,
...
static int indexupvalue(FuncState *fs, TString *name, expdesc *v) {
	...
	/* new one */luaY_checklimit(fs, f->nups + 1, LUAI_MAXUPVALUES, "upvalues");
...
static void leaveblock(FuncState *fs) {
	...
	/* a block either controls scope or breaks (never both) */lua_assert(!bl->isbreakable || !bl->upval);
	...
	if (ls->t.token != ')') { /* is `parlist' not empty? */
		do {
			...
			case TK_DOTS: { /* param -> `...' */
				...
				/* use `arg' as default name */new_localvarliteral(ls, "arg", nparams++);
...
static void forlist(LexState *ls, TString *indexname) {
	...
	int line;
	int base = fs->freereg;
	/* create control variables */new_localvarliteral(ls, "(for generator)", nvars++);
...
static void localfunc(LexState *ls) {
	...
	/* debug information will only see the variable after this point! */getlocvar(fs, fs->nactvar - 1).startpc
			= fs->pc;
}

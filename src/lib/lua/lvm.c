...
void luaV_concat(lua_State *L, int total, int last) {
	do {
		...
		if (!(ttisstring(top-2) || ttisnumber(top-2)) || !tostring(L, top-1)) {
			...
			/* at least two string values; get as many as possible */size_t tl =
					tsvalue(top-1)->len;
			char *buffer;
			int i;
...
static void Arith(lua_State *L, StkId ra, const TValue *rb, const TValue *rc,
	...
	TValue tempb, tempc;
	const TValue *b, *c;
	if ((b = luaV_tonumber(rb, &tempb)) != NULL && (c = luaV_tonumber(rc,
			&tempc)) != NULL) {
...
void luaV_execute(lua_State *L, int nexeccalls) {
	...
	reentry: /* entry point */lua_assert(isLua(L->ci));
	pc = L->savedpc;

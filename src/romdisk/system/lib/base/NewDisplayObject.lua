...
function M:getMatrix(target)
	...
	if self.__transform and not self.matrix then
		if self.__anchor or self.__translate then
			m = m:translate(self.x, self.y)
		end
		if self.__rotate then
			m = m:rotate(self.rotation)
		end
		if self.__anchor then
			m = m:translate(-self.anchorx * self.scalex, -self.anchory * self.scaley)
		end
		if self.__scale then
			m = m:scale(self.scalex, self.scaley)
		end
	end
